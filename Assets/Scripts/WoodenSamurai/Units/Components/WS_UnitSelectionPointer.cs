...
[RequireComponent(typeof(WS_Unit))]
public class WS_UnitSelectionPointer : MonoBehaviour, IPointerEnterHandler, IPointerClickHandler, IPointerExitHandler
{
    [Inject] private WS_TypeGameManager _gameManager;
    [Inject] private WS_ContainerStatusGame _statusGame;
    [Inject] private WS_SelectionMaterialManager _selectionMaterials;
    ...
    private WS_Unit _targetUnitEnter;

    public event Action<WS_Unit> OnUnitSelected;
    public event Action<WS_Unit> OnUnitEnter;
    public event Action<WS_Unit> OnUnitExit;

    private void Awake()
    {
        _unit = GetComponent<WS_Unit>();
    ...
    private void Start()
    {
        ...
        _selectionMaterials.MaterialChangedEvent += MaterialChangedUpdate;
    }

    private void MaterialChangedUpdate(Material obj)
    {
        MaterialUpdate();
    }
    ...
    {
        if (_meshRenderer.material == null) Debug.LogError($"У {this} - нет материала _meshRendererCloth");
        if (_unit.Player == EnumPlayers.PlayerOne) _originalMaterial = _meshRenderer.material = _selectionMaterials.PlayerOne;
        else _originalMaterial = _meshRenderer.material = _selectionMaterials.PlayerTwo;
    }
    ...
    private void MainSelectAction(bool selected, bool setActionTarget, WS_EnumStatusGame nextStatusGame, Material setMaterial)
    {
    ...
    {
        MainSelectAction(false, true, WS_EnumStatusGame.Hit, _originalMaterial);
        ResetOldUnit();
    ...
    private void Select()
    {
        MainSelectAction(true, false, WS_EnumStatusGame.SelectedUnit, _selectionMaterials.GetFocusMaterialUnit);
        OnUnitSelected?.Invoke(_unit);
        ResetOldUnit();
    ...
    private void ResetOldUnit()
    {
        ...
        if (_statusGame.Status == WS_EnumStatusGame.SelectedCell
            && _unit.StatusUnit == WS_EnumStatusUnitEnemy.My)
            _meshRenderer.material = _originalMaterial;
    }

    private void ResetMaterialOldUnit(WS_EnumStatusGame obj) => ResetOldUnit();
    ...
    private void Enter()
    {
        ...
        if (_unit.StatusUnit == WS_EnumStatusUnitEnemy.Enemy && _statusGame.Status == WS_EnumStatusGame.SelectedUnit)
        {
            ...
            // Курсор - Атака
            if (_oldSelectedUnitStatic._unit.Class == WS_EnumStatusUnitClass.Samurai) _animatedCursor.SetCursorState(EnumStatusCursor.Attack);
        ...
        else if (_unit.StatusUnit == WS_EnumStatusUnitEnemy.Enemy && _statusGame.Status != WS_EnumStatusGame.SelectedUnit)
        {
    ...
    private void Click()
    {
        ...
        if (_statusGame.Status == WS_EnumStatusGame.SelectedUnit
            && _unit.StatusUnit == WS_EnumStatusUnitEnemy.Enemy)
        {
        ...
        else if ((_statusGame.Status == WS_EnumStatusGame.SelectedUnit ||
            _statusGame.Status == WS_EnumStatusGame.Empty)
            && _unit.StatusUnit == WS_EnumStatusUnitEnemy.My)
        {
            // Сюда можно добавлять любые действия
    ...
    private void Exit()
    {
        ...
        if (!_selected || (_selected && _unit.StatusUnit == WS_EnumStatusUnitEnemy.Enemy) || (_oldSelectedUnitStatic != this && _unit.StatusUnit == WS_EnumStatusUnitEnemy.My))
            _meshRenderer.material = _originalMaterial;
    }
    ...
    private void SetInjectionAndValidateDependencies()
    {
        ...
        {
            _gameData = FindObjectOfType<WS_GameData>();
            _gameManager = FindObjectOfType<WS_TypeGameManager>();
            _statusGame = FindObjectOfType<WS_ContainerStatusGame>();
            _selectionMaterials = FindObjectOfType<WS_SelectionMaterialManager>();
            _moveSystem = FindObjectOfType<WS_MoveSystem>();
            _animatedCursor = FindObjectOfType<AdvancedCursorController>();
            _soundUnit = FindObjectOfType<WS_SoundsUnit>();
        }
