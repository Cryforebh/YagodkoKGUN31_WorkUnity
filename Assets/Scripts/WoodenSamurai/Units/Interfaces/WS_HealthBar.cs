...
public class WS_HealthBar : MonoBehaviour, IPointerEnterHandler, IPointerClickHandler, IPointerExitHandler
{
    [Inject] private WS_StatisticsUnitsVisualManager _healthBarManager;
    ...
    private void Awake()
    {
        _healthBarManager = FindObjectOfType<WS_StatisticsUnitsVisualManager>();
    ...
    private void Enter()
    {
        ...
        if (_unit.ModifierDamage > 0 && _unit.Class == WS_EnumStatusUnitClass.Samurai) _healthBarManager.ModifierDamage.enabled = true;
        if (_unit.ModifierDrowRange && _unit.Class == WS_EnumStatusUnitClass.Ranger) _healthBarManager.ModifierDrowRange.enabled = true;
    }
